<template>
    <div class="container">
        ...
        <div class="add-lang-container">
            <el-autocomplete
                v-model="mExplorer.mAddLangInput"
                :fetch-suggestions="mExplorer.fFetchLangSug"
                placeholder="创建语言资源"
            />
            ...
            <template #default="{ node }">
                <span class="tree-node">
                    ...
                    <span style="flex: 1; display: flex; align-items: center">
                        ...
                        <!-- 编辑时候的*号 -->
                        <span v-if="!node.data.isDir && Object.keys(mEditor.mChangeData[node.label] || {}).length > 0"
                            >*</span
                        >

                        <!-- 进度条 -->
                        <el-progress
                            v-if="!node.data.isDir && node.data.totalKeys > 0"
                            :percentage="(node.data.currentKeys * 100) / node.data.totalKeys"
                            :format="(p) => p.toFixed(0) + '%'"
                            style="margin-left: auto; width: 70px"
                            :stroke-width="4"
...
import { ElInput, ElAutocomplete, ElButton, ElTree, ElIcon, ElProgress } from 'element-plus';
import { Search, Document, Folder, FolderOpened, Setting } from '@element-plus/icons-vue';

import { watch } from 'vue';

import { settings } from '../dialogs/dialogs';
